...
func PlanString() string {
	...
	var bytes []byte
	var err error
	if bytes, err = json.Marshal(plan.FluidNodeToPlan(root)); err != nil {
		panic(err)
	}
	return string(bytes)
}

func ReadBinaryFile(path string) (root fluid.Node) {
...
func ReadBinaryPlan(reader io.Reader) (root fluid.Node) {
	...
	if root, err = fluid.ReadRootNode(msg); err != nil {
		panic(err)
	}
	return
}
...
func FindFirstNodeByType(node *fluid.Node, opType fluid.OperatorType) (target *fluid.Node, found bool) {
	if node == nil {
		return nil, false
	}
	if node.Type() == opType {
...
func WriteJsonFile(root *fluid.Node, filePath string) {
	CreateFile(WriteJson(root), filePath)
}

func WriteJson(root *fluid.Node) (bytes []byte) {
	var err error
	if bytes, err = json.Marshal(plan.FluidNodeToPlan(*root)); err != nil {
		panic(err)
	}
