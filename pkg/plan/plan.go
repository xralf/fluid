...
func FluidNodeToPlan(node fluid.Node) (p PlanNode) {
	var err error
	p.Id = node.Id()
	...
	}
	{
		...
		for i := 0; i < calls.Len(); i++ {
			...
			var inputTypes capnp.EnumList[fluid.FieldType]
			...
			var properties capnp.StructList[fluid.FunctionProperty]
	...
	// var fieldConstantConditions capnp.StructList[fluid.FieldConstantCondition]
	...
	// 		panic(err)
	// 	}
	// 	comparator := fluidComparatorToString(fieldConstantConditions.At(i).Comparator())
	...
	// 	comparator := fluidComparatorToString(fieldFieldCondition.At(i).Comparator())
	...
	var children capnp.StructList[fluid.Node]
